...
class DrewPlotWidget(QLabel):
    def __init__(self,parent):
        super(DrewPlotWidget, self).__init__(parent)
        self.rgb = None
        self.project_name = None
        # {projectname:{rgb:,page:data}}
        self.project = {}
    ...
    def paintEvent(self, event):
        if self.drew_flag:
            painter = QPainter()
            painter.begin(self)
            pen = QPen(QColor(self.rgb[0],self.rgb[1],self.rgb[2]), 2, Qt.SolidLine)
            painter.setPen(pen)
            '''
